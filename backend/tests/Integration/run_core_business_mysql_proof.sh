...
checkout="$(status -H "$auth" -H 'content-type: application/json' -d '{"builderId":1,"planId":3,"successUrl":"https://success.example","cancelUrl":"https://cancel.example"}' "http://127.0.0.1:$api/api/v1/subscriptions/payments/sessions")"; [[ ${checkout%%|*} == 201 ]]; grep -q 'cs_runtime' <<<"${checkout#*|}"; ! grep -q 'rk_test_minimum' <<<"${checkout#*|}"
...
invoices="$(status -H "$auth" "http://127.0.0.1:$api/api/v1/subscriptions/payments/invoices?builderId=1")"; [[ ${invoices%%|*} == 200 ]]; grep -q 'in_runtime' <<<"${invoices#*|}"
